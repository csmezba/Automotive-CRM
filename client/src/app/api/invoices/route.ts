...
export async function GET(req: NextRequest) {
  try {
    ...
    const expressRes = await fetch(`${EXPRESS_URL}/api/invoices${url.search}`, { cache: "no-store" });
    if (expressRes.ok) {
      const data = await expressRes.json();
      return NextResponse.json(data);
    }
    return NextResponse.json({ error: "Failed to fetch invoices from Express" }, { status: expressRes.status });
  } catch (e: any) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const expressRes = await fetch(`${EXPRESS_URL}/api/invoices`, {
      method: "POST",
