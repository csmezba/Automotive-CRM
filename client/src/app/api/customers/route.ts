...
export async function GET(req: NextRequest) {
  try {
    ...
    const expressRes = await fetch(`${EXPRESS_URL}/api/customers${url.search}`, { cache: "no-store" });
    if (expressRes.ok) {
      const data = await expressRes.json();
      return NextResponse.json(data);
    }
    return NextResponse.json({ error: "Failed to fetch customers from Express" }, { status: expressRes.status });
  } catch (e: any) {
